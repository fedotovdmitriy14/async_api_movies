...
async def test_get_film(es_client, client_session):
    ...
    response = await make_get_request(client_session, method=f'{index}/{film_id}')
    assert response.get('status') == 200
...
async def test_get_cache_film(client_session, es_client):
    ...
    film_id = data[0].get('id')
    response_1 = await make_get_request(client_session, method=f'{index}/{film_id}')
    assert response_1.get('status') == HTTPStatus.OK
    await es_client.delete('movies', film_id)
    response_2 = await make_get_request(client_session, method=f'{index}/{film_id}')
